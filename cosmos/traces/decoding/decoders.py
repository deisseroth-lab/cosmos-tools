...
try:
    ...
    from keras.layers import Dense, LSTM, SimpleRNN, GRU, Activation, Dropout,\
        Conv1D, Dense, MaxPooling1D, Flatten, Input
...
#     """
#     Define a custom metric that 
#     averages within each group before 
#     computing accuracy as average across them 
#     (as opposed to averaging across all datapoints).
#     """
#     import keras.backend as K
    
#     confusion = K.confusion_matrix(y_true, y_pred)
#     accuracy_unweighted = np.sum(np.diag(confusion))/np.sum(confusion[:])
#     confusion = confusion/confusion.sum(1)[:, None]
#     accuracy_weighted = K.mean(K.diag_part(confusion))
...
class DenseNNDecoder(object):
    ...
    units: integer or vector of integers, optional, default 400
        ...
        If you want multiple layers, input a vector (e.g. units=[400,200]) will give you 2 hidden layers with 400 and 200 
        units, repsectively.
    ...
    def fit(self,X_flat_train,y_train,out_activation=None,loss='mse',
            layer_activation='relu',fit_kwargs={}, l1=None, hidden_layers=False):
        ...
        if do_l1:
            model.add(Dense(y_train.shape[1],
                            kernel_regularizer=regularizers.l1(l1)))  #Add final dense layer (connected to outputs)
        else:
        ...
        model.compile(loss=loss,optimizer='adam', weighted_metrics=['accuracy']) #Set loss function and optimizer
        # history = model.fit(X_flat_train,y_train,nb_epoch=self.num_epochs,verbose=self.verbose,**fit_kwargs) #Fit the model
        history = model.fit(X_flat_train,y_train,epochs=self.num_epochs,verbose=self.verbose,**fit_kwargs) #Fit the model
        self.model=model
...
class ConvNetDecoder(object):

    """
    Class for the (1D) CNN decoder
    ...
    def __init__(self,filter_length,nb_filter=10,dropout=0,num_epochs=10,verbose=0):
         self.dropout=dropout
         self.num_epochs=num_epochs
         self.verbose=verbose
         ...
         #Determine the number of hidden layers (based on "units" that the user entered)
         self.num_layers=len(nb_filter)

    def fit(self, X_train, y_train, out_activation='softmax', loss='mse', metrics=['accuracy']):
        ...
        y_train: numpy 3d array of shape [n_samples, n_time_bins, n_outputs]
            This is the outputs that are being predicted
        """

        #model=Sequential() #Declare model
        #model.add(Input(shape=(None, 1))) # not X_train.shape[-1] i.e. # neurons??
        F = Input(shape=(None, X_train.shape[-1]))
        x = F
        for nf in self.nb_filter:
            x = Conv1D(nb_filter=nf,
        ...
        s = Conv1D(nb_filter = y_train.shape[2],
                   filter_length = 1,
                   input_shape = (self.nb_filter[-1], None), #(99, 99), #(self.nb_filter[-1], None), #
                   activation=out_activation,
                   padding = 'same')(x)


        #s = Conv1D(1,1, activation=out_activation, padding='same')(x)
        #x = Flatten()(x)
        # #Add dense connections to all outputs
        #s = Dense(y_train.shape[1])(x) #Add final dense layer (connected to outputs)
        #if out_activation is not None:
        #     s = Activation(out_activation)(s)
        #Fit model (and set fitting parameters)
        model = Model(inputs=F, outputs=s)
        model.compile(loss=loss,optimizer='adam',metrics=metrics) #Set loss function and optimizer
...
class LSTMDecoder(object):
    ...
    def fit(self,X_train,y_train,loss='mse',out_activation=None, metrics=['accuracy']):
        ...
        #Fit model (and set fitting parameters)
        model.compile(loss=loss,optimizer='rmsprop',metrics=metrics) #Set loss function and optimizer
        model.fit(X_train,y_train,nb_epoch=self.num_epochs,verbose=self.verbose) #Fit the model
        self.model=model
