...
def partition_dataset_subset(rates, BD, which_subset,
    ...
    :param which_subset: string that indicates which subset of the data to use.
                         ...
                         - 'post_odor': 2 seconds beginning 0.5s after odor.
                         - 'block_start':
                            Only use trials from the first half of a block.
                         - 'block_end':
                            Only use trials from the second half of a block.
    :param fraction_test: float.
                        Fraction of the total trials that should be assigned
                        to the test dataset.
    ...
    if which_subset == 'all':
        ...
        data_split, trials_split = partition_dataset(
            rates, trial_types, fraction_test=fraction_test,
            rand_seed=rand_seed, do_debug=do_debug)
        ...
        data_split, trials_split = partition_dataset(
            subset_rates, trial_types, fraction_test=fraction_test,
            rand_seed=rand_seed, do_debug=do_debug)
        ...
        data_split, trials_split = partition_dataset(
            subset_rates,
            trial_types[which_trials], fraction_test=fraction_test,
            rand_seed=rand_seed, do_debug=do_debug)
        ...
        data_split, trials_split = partition_dataset(
            subset_rates, trial_types[which_trials],
...
def partition_dataset(rates, trial_types, fraction_test=0.5, rand_seed=0,
    ...
    :param trial_types: [ntrials]. The type of each trial. So that you
                        can ensure that test and train subsets each get equal
                        splits of all of the trial types.
    :param fraction_test: float. Fraction of the total trials that
                          should be assigned to the test dataset.
    :return: data_split: a dict. keys: 'test' or 'train'.
                                 vals: The associated rates matrices.
    :return trials_split: a dict. keys: 'test' or 'train'.
                                  vals: The associated trial indices.
    ...
    if method == 'grid':
        ...
        for hemisphere in [0, 1]:
            for region_name in ['MO', 'PTLp', 'RSP', 'SSp', 'VIS']:
                ...
                in_region = np.array(
                    CT.region_of_cell)[:, np.newaxis] == region_id
...
def get_regression_performance(results):
    ...
    yr = np.log(np.abs(ytrue / (ypred+1e-16) + 1e-16))
    ys = np.sum(ytrue * np.log(np.abs(ytrue / (null_pred+1e-16) + 1e-16)) -
                ytrue + null_pred)
    ed_poisson = (1 - (2 * np.sum(
        ytrue * yr - ytrue + ypred))/(2 * ys))
    performance = {}
...
def print_regression_performance(results, performance, fit_opts):
    ...
    """
    res_string = ('Target: {}, train_group: {}, rank: {}' +
                  ', alpha: {}, target_group: {}')
    print(res_string.format(
            results['target'],
            results['train_group'],
...
def organize_peer_prediction_results(out):
    ...
    for i in range(len(out['test_scores'])):
        ...
        param = out['params'][i]
        neuron_ind = np.where(
            np.array(out['target_list']) == param['target'])[0]
        sv_list = np.array(out['singularval_list'])
        sv_ind = np.where(sv_list == param['fit_opts']['ncomponents'])[0]
    ...
    for ind, target_group in enumerate(use_groups[:]):
        ...
        if do_bar_chart:
            ...
            if group_centroid[ind, 0] > 0:
                ...
                utils.centroids_on_atlas(target_score,
                                         np.arange(len(target_score)),
                                         group_centroid,
                                         atlas_tform, max_radius=5000,
                                         highlight_inds=np.array(ind),
                                         vmin=0, vmax=0.1, fill_highlight=True)
                plt.title('Group {}, rank {}, expt {}'.format(
                    target_group, singularval_list[rank_ind], expt_num))
    if not do_bar_chart:
