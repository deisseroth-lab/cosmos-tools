...
def plot_task_classification_for_one_dataset(dataset_id, CT_ind,
    ...
    n_components = 40
    sets = [
            {'method': 'nmf', 'protocol': 'full', 'randseed': 1,
             'n_components': n_components, 'l1': 0.0},
            {'method': 'classify', 'protocol': 'mr2',
             'randseed': '', 'n_components': '', 'l1': ''},
        ]
    all_nmf = load_clustering_results(
        dataset_id, sets, clustering_dir, protocol_as_key=True)
    ...
    if protocol == '4way':
        rates = gaussian_filter1d(
            allCT[CT_ind].St, 1.5, axis=1, mode='constant')
        event_frames = allCT[CT_ind].event_frames
        data = concatenate_trial_type_avgs(
            all_nmf['4way']['trial_sets'], rates, do_plot=False)
    elif protocol == '2way':
        rates = gaussian_filter1d(
            allCT[CT_ind].St, 1.5, axis=1, mode='constant')
        do_super_clust = True
        if not do_load:
            data = concatenate_trial_type_avgs(
                all_nmf['2way']['trial_sets'], rates, do_plot=True)
    elif protocol == 'oeg2way':
        pass
        ...
        event_frames = allCT[CT_ind].event_frames
        data = concatenate_trial_type_avgs(
            all_nmf['glm']['trial_sets'], rates, do_plot=False)
        ...
        event_frames = allCT[CT_ind].event_frames
        data = concatenate_trial_type_avgs(
            all_nmf['mr2']['trial_sets'], rates, do_plot=False)
        data_front = concatenate_trial_type_avgs(
            all_nmf['mr2']['trial_sets'], rates,
            do_plot=False, get_first_half=True)
        data_back = concatenate_trial_type_avgs(
            all_nmf['mr2']['trial_sets'], rates,
    ...
    if 'mr2' in all_nmf.keys():
        source_coloring = assign_colors_to_sources(
            all_nmf['mr2']['ordered_clustering'],
            all_nmf['mr2']['ordered_super_clustering'],
            cmap='jet', same_within_super_cluster=True,
            specify_discrete_colors=True)

        trial_set_inds = all_nmf['mr2']['trial_set_inds']
        trial_names = all_nmf['mr2']['trial_names']
        label_positions = trial_set_inds + np.diff(trial_set_inds)[0]/2
        cm = 'plasma'
        plot_clustered_sources(
            all_nmf['mr2']['ordered_clustering'],
            all_nmf['mr2']['ordered_super_clustering'],
            source_means=data_back,
    ...
    if 'mr2' in all_nmf.keys():
        plot_cluster_averages(
            data, all_nmf['mr2']['ordered_clustering'],
            all_nmf['mr2']['ordered_super_clustering'],
            trial_set_inds, by_trial_type=True,
            event_frames=[event_frames[1], event_frames[1]+2*fps],
            event_labels=[0, 2],
            time_labels={
                'labels': ['0']*len(trial_names) + ['2']*len(trial_names),
                'positions': np.hstack(
                    (trial_set_inds+event_frames[1],
                     trial_set_inds+event_frames[1]+2*fps))},
            vertical_lines={'k': trial_set_inds,
        ...
        plt.gcf().set_size_inches(w=2.819, h=0.4)
    ...
    # For each super cluster, plot the location of the included cells
    # with the corresponding color from the above plot.
    if 'mr2' in all_nmf.keys():
        subplot_titles = None
        plot_cluster_spatial_maps(
            source_coloring, all_nmf['mr2']['ordered_super_clustering'],
        ...
        if subplot_titles is None:
            plt.gcf().set_size_inches(w=3.7, h=2)
        else:
            plt.gcf().set_size_inches(w=2.5, h=2)

        plt.savefig(os.path.join(clust_fig_dir, savename),
                    transparent=True, rasterized=True, dpi=600)

    # Plot single trials for the schematic
    ...
    savename = (
        'id' + str(dataset_id) + '_' + protocol +
        '_single_trial_' + str(source_id) + '.pdf')
    plt.savefig(os.path.join(clust_fig_dir, savename),
                transparent=True, rasterized=True, dpi=600)

    # Now plot mean traces
    plt.figure()
    plot_trial_type_mean_traces(
        source_id, rates, trial_types, event_frame, fps)

    plt.gcf().set_size_inches(w=1.5, h=3)  # Control size of figure in inches
    savename = (
        'id' + str(dataset_id) + '_' + protocol + '_single_trial_means_' +
        str(source_id) + '.pdf')
    plt.savefig(os.path.join(clust_fig_dir, savename),
                transparent=True, rasterized=True, dpi=600)
