...
def event_rate(event_times, bin_size, max_time):
    ...
    for j in t:
        ...
        rate[k] = np.sum(np.logical_and(event_times >= curr_win[0],
                                        event_times < curr_win[1])) / bin_size
        k = k + 1
...
def get_state_times(allS, state_name, ind=0):
    ...
    :param allS: either a single bpod data dict or array of dicts
    :param state_name: name of the state you are querying
    :param ind: if want the end of the state instead of the beginning, ind=1
    :return: state_times: an array with the time for each trial.
                          Entry is nan if did not enter state on trial.
    """
    if isinstance(allS, dict):
        ...
        for i in range(len(allS)):
            ...
            state_times = np.concatenate(
                (state_times, out)) if state_times.size else out
...
def _get_state_times(S, state_name, ind=0):
    ...
    :param state_name: name of the state: 'Reward', 'NoReward'
    :param ind: beginning of state = 0, end = 1, default = 0
    :return: state_times: an array with the time for each trial.
    """
...
def get_lick_rates(allS, bin_size=1.0 / 15, max_time=10, port='Port1In'):
    ...
    (concatenates multiple sessions if necessary).
    Note: rate[0] is the rate of licks in the bin from time=0 to time=binSize

    Wrapper function to concatenate together _get_lick_rates
    for multiple sessions (where the multiple sessions
    are exactly the same and taken on the same day, but were split up by bpod).
    :param allS: either a single bpod data dict or array of dicts
         (loaded using load_bpod_data).
    :param binSize: in seconds
    :param maxTime: in seconds
    ...
    :return all_lick_rates: a matrix [trial x time] containing lick rates
            ...
            all_lick_times: a list containing, for each session,
            a dict containing actual lick times for each trial
    """
    print('lick rates bin size: ' + str(bin_size))
    if isinstance(allS, dict):
        return _get_lick_rates(
            allS, bin_size=bin_size, max_time=max_time, port=port)
    else:
        ...
        for i in range(len(allS)):
            out = _get_lick_rates(
                S=allS[i], bin_size=bin_size, max_time=max_time, port=port)
            all_lick_rates = np.concatenate(
                (all_lick_rates, out[0])) if all_lick_rates.size else out[0]
            t = out[1]
...
def _get_lick_rates(S, bin_size=1.0 / 15, max_time=10, port='Port1In'):
    ...
    all_lick_times = {}
    ntrials = len(S['RawEvents']['Trial'])
    all_lick_rates = np.zeros(
        (ntrials, int(np.ceil((max_time - bin_size) / bin_size))))
    for trial in range(ntrials):
...
def make_bpod_data_path(base_path, mouse_name, protocol_name, date, session):
    ...
    """
    fname = (mouse_name + '_' + protocol_name +
             '_' + date + '_Session' + str(session) + '.mat')
...
def make_bpod_processed_img_paths(
        img_path, processed_img_path, bpod_img_fullpaths):
    """
    Returns a list with paths to the directory where processed (i.e. motion
    corrected) videos corresponding to each behavior trial that will be saved.
    """
    processed_paths = []
