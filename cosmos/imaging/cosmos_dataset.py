...
class CosmosDataset:
    ...
    def __init__(self, base_path, dataset_info, fig_save_path):
        ...
        self.base_path = base_path  # Top directory where CNMFe out.mat is located.
        self.fig_save_path = fig_save_path  # Top directory for saving figures (i.e. dropbox).
        ...
        self.included = dict()  # For 'bot_focus' and 'top_focus',
                                # which cells are included after merging.
                                # assumes that this is using culled traces.
    ...
    def cull(self, do_load=True, do_auto=False, do_manual=True, which_key=None,
        ...
        :param only_show_non_culled: Bool. If True, then does not display neurons
                                     that have been previously culled (i.e. either
                                     from auto-culling or a previous run of manual
                                     culling).
        ...
        for key, d in self.data.items():
            ...
            footprints = np.reshape(footprints,
                                    (base_im.shape[0], base_im.shape[1], ncells), order='F')
            ...
            if do_auto:
                ...
                keep_cells, corrs, aspect_ratios = utils.classify_neurons(results,
                                                                          corr_thresh,
                                                                          aspect_ratio_thresh)
            ...
            if do_manual:
                ...
                if keep_cells is None:
                    ...
                    keep_cells = 1 * np.ones((ncells, 1))  # Passed in by reference to GUI.
    ...
    def plot_cells_premerge(self, cell_ids, do_traces=True,
        ...
        for key, d in self.data.items():
            ...
            if use_culled_cells:
                f = np.load(os.path.join(fpath.rsplit("/", 1)[0], 'keep_cells.npz'))
                which_cells = np.where(f['keep_cells'] > 0)[0]
            else:
                which_cells = np.arange(results['C'].shape[0])

            # Gather traces and footprints.
            S = np.array(results['S'].todense())[which_cells,:]  # Inferred spikes
            C = np.array(results['C'])[which_cells,:]  # Denoised fluorescence
            F = np.array(results['C_raw'])[which_cells, :]  # Raw fluorescence
            base_im = results['meanFrame']
            ff = np.asarray(results['A'].todense())
            ff = np.reshape(ff, (base_im.shape[0], base_im.shape[1], ff.shape[1]), \
                            order='F') 
            ff = ff[:, :, which_cells]

            # Initialize CellPlotter class with data.
            CP = cell_plotter.CellPlotter(C, F, ff, base_im, date=self.date,
                                   name=self.name, fig_save_path=self.fig_save_path, 
                                   suffix='premerge_'+key+'.pdf')
            CP.set_highlighted_neurons(cell_ids)
            fig = plt.figure(figsize=(20,10))
            if do_traces:
                CP.plot_traces(n_timepoints=n_timepoints, ax=plt.subplot(121), \
                               save_plot=False)
            if do_contours:
    ...
    def align_planes(self, use_culled_cells=True, do_debug=False):
        ...
        if do_alignment:
            ...
            for key in ['bot_focus', 'top_focus']:
                ...
                f = np.load(os.path.join(fpath.rsplit("/", 1)[0], 'keypoints.npz'))
                ...
                footprints_unalign[key] = np.reshape(footprints, 
                                            (mean_frames_unalign[key].shape[0], 
                                             mean_frames_unalign[key].shape[1], 
                                             footprints.shape[1]), 
                                             order='F')

                cm_unalign[key], self.areas[key] = utils.get_cm(footprints_unalign[key])

                self.tseries[key] = np.array(results['C'])[which_cells,:] 
                self.spikes[key] =  np.array(results['S'].todense())[which_cells,:]
                self.tseries_raw[key] = np.array(results['C_raw'])[which_cells, :]
            ...
            aligned_keypoint = utils.transform_points(self.tform,
                                                      keypoints_unalign['bot_focus'])
            self.keypoints_aligned['top_focus'] = keypoints_unalign['top_focus']
            self.keypoints_aligned['bot_focus'] = aligned_keypoint

            print('Aligning mean frames.')
            aligned_frame = utils.align_image(self.tform, mean_frames_unalign['bot_focus'])
            aligned_frames = utils.crop_images(mean_frames_unalign['top_focus'],
                                               aligned_frame)
            ...
            aligned_atlases = utils.crop_images(self.atlas_info['aligned_atlas_outline'],
                                                aligned_frames[0])
            self.atlas_info['aligned_atlas_outline'] = aligned_atlases[0]
            
            
            print('Aligning corr frames.')
            aligned_frame = utils.align_image(self.tform, corr_frames_unalign['bot_focus'])
            aligned_frames = utils.crop_images(corr_frames_unalign['top_focus'],
                                               aligned_frame)
            ...
            aligned_footprint = utils.align_stacks(self.tform, footprints_unalign['bot_focus']) 
            aligned_footprints = utils.crop_stacks(footprints_unalign['top_focus'],
                                                   aligned_footprint)
            ...
            aligned_cm = np.fliplr(utils.transform_points(self.tform,
                                                          np.fliplr(cm_unalign['bot_focus'])))
            self.cm_aligned['top_focus'] = cm_unalign['top_focus']
            self.cm_aligned['bot_focus'] = aligned_cm

            self.roi_map_aligned['top_focus'] = utils.get_roi_map(self.cm_aligned['top_focus'],
                                                  self.areas['top_focus'],
                                                  self.mean_frames_aligned['top_focus'].shape)   
            self.roi_map_aligned['bot_focus'] = utils.get_roi_map(self.cm_aligned['bot_focus'],
                                                  self.areas['bot_focus'],
                                                  self.mean_frames_aligned['bot_focus'].shape)  
            
            # Potentially optional save-out of unaligned variables, for testing purposes.
    ...
    def merge_planes(self, do_debug=False, do_plots=False, manual_keypoints=None):
        """
        ...
        ### Compute roi sizes from each image within different bands
        ...
        if do_merge:
            left_classifier, right_classifier, blocks_inds = \
                    self.get_classification_lines(nblocks=10,
                                                  do_plot_classification_lines=do_plots,
            ...
            for key in ['top_focus', 'bot_focus']:
                ...
                cm = self.cm_aligned[key] ### THIS SHOULD ONLY USE KEEP CELLS??? ENFORCE THIS?
                included[key] = self._classify_points(cm[:, 1], cm[:, 0],
                                                left_classifier,
                                                right_classifier,
                                                is_inside)
            ...
            if do_plot_separate:
                for key in ['top_focus', 'bot_focus']:
                    ...
                    c = p1.circle(x=cm[:,1], y=cm[:,0], radius=area[:, 0]/300.0, color='blue')
                    c = p1.circle(x=np.squeeze(cm[incl,1]), y=np.squeeze(cm[incl,0]),\
                                  radius=np.squeeze(area[incl, 0])/300.0, color='firebrick')
                    ...
                    lfit = p1.line(y=yy_plot_left , x=xx, color='green', line_width=3)
                    rfit = p1.line(y=yy_plot_right, x=xx, color='green', line_width=3)
                    show(p1)
                    p1.output_backend = "svg"
                    export_svgs(p1, filename=os.path.join(self.fig_save_path, self.date, self.name, 
                                                          "roi_"+key+".svg"))
    ...
    def get_classification_lines(self, nblocks=10,
        ...
        all_blocks, blocks_inds = self._get_block_projections(nblocks,
                                             ['top_focus', 'bot_focus'],
                                             self.roi_map_aligned)
        print(blocks_inds)
        do_plot_blocks_raw = False
        ...
        if manual_keypoints is None:
            ...
            for i in range(nblocks):
                block_means = dict()
                for key in ['top_focus', 'bot_focus']:
                    block_means[key] = trace_utils.moving_avg(all_blocks[key][:,i], \
                                                        do_median)
                L = min([len(val) for (key, val) in block_means.items()])
                for key in ['top_focus', 'bot_focus']:
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore",  np.RankWarning)
                        block_means[key] = savgol_filter(block_means[key][:L], 21, 3)
                ...
                if doOrig:
                    inds = np.where(block_means['bot_focus'] > block_means['top_focus'])[0]
                    if len(inds) == 0:
                        inds = np.array([0, L-1])
                    ...
                    block_means['bot_focus'] = self._interp_nans(block_means['bot_focus'])
                    block_means['top_focus'] = self._interp_nans(block_means['top_focus'])

                    intersections = np.where(np.diff(np.sign(block_means['bot_focus']
                                                             - block_means['top_focus'])))[0]
                    d = 20
                    intersections = intersections[np.where(np.logical_and(intersections > d,
                                                                          intersections < L-d))[0]]
            ...
            yy_plot_right, classifier_right = utils.fit_polynomial(xx, yy, xx, 2)
            ...
            if do_plot_classification_lines:
                for key in ['top_focus', 'bot_focus']:
                    ...
                    c = p1.circle(x=cm[:, 1], y=cm[:, 0],
                                  radius=area[:, 0] / 300.0)
                    l = p1.line(y=left_inds[1:], x=blocks_inds[key][1:],
                                color='orange')
                    ...
                    #                 show(p1)
                    p1.output_backend = "svg"
                    export_svgs(p1, filename=os.path.join(self.fig_save_path,
                                                          self.date, self.name,
                                                          "classification_lines_" + key + ".svg"))
                    show(p1)
        else:
    ...
    def save_merged(self):
        ...
        if do_merge:
            ...
            save_focmask=False
            if save_focmask:
                focmask_path = os.path.join(self.base_path, self.date, self.name,
                                            'focus_incl_masks')
            ...
            merged_tseries = np.vstack((self.tseries['top_focus'][incl_t, :], \
                                        self.tseries['bot_focus'][incl_b, :]))

            merged_spikes = np.vstack((self.spikes['top_focus'][incl_t, :], \
            ...
            merged_focus_id = np.vstack((np.zeros( (len(incl_t), 1) ),\
                                         np.ones( (len(incl_b), 1) )))
        ...
        try:
            led_frames = np.load(os.path.join(self.base_path, self.date, self.name, 'led_frames.npz'))['inds']
        except:
        ...
        with h5py.File(save_path, "w") as hf:
            hf.create_dataset("footprints", data=merged_footprints, compression="gzip", compression_opts=9)
    ...
    def plot_cells_postmerge(self, cell_ids, do_traces=True,
            ...
            :param do_atlas_overlay: bool. Overlay aligned atlas on contour plot.
            :param highlight_neurons: bool. Highlight specified cell_ids in the
                                      contour plot.
            :param n_timepoints: int. How many frames to plot in the trace plot.
            :param do_events: bool. Plot vertical lines at led_frame times.
            :param do_seconds: bool. Make x-axis in seconds (as opposed to frames).
            """
            ...
            CP = cell_plotter.CellPlotter(C, F, ff, base_im, date=self.date, name=self.name,
    ...
    def cull_postmerge(self, do_load=True, do_auto=False, do_manual=True,
        ...
        :param which_key: If only one of the datasets in self.data
                          should be run. (i.e. 'top_focus', 'bot_focus',
                          'two_photon', or 'single_focus').
        :param nt: How many timepoints to display in the traces. Fewer timepoints
                   leads to faster loading and display.
        :param only_show_non_culled: Bool. If True, then does not display neurons
                                     that have been previously culled (i.e. either
                                     from auto-culling or a previous run of manual
                                     culling).
...
############# Private functions ################################################
    ...
    def _load_dataset(self):
        ...
        data['single_focus'] = self._load_dataset_mat(target_name='single_out.mat')
    ...
    def _load_dataset_mat(self, target_name='out.mat'):
        ...
        :param datasets: a list of dictionaries containing keys 'date' and 'name'
    ...
    def _classify_points(self, xcm, ycm, classifier_left, classifier_right, is_inside):
        """
        ...
        :params classifier_left: left sklearn model that defines classification line
