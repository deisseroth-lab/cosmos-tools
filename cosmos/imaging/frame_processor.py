...
class FrameProcessor:
    ...
    Attributes:
        ...
        led_frame_inds: ndarray.
            Indices of frames with trial-start indicating LED flash.
        alignment_keypoints: ndarray.
            Keypoints for aligning cropped regions to each other.
    ...
    def __init__(self, raw_data_dir, processed_data_dir, dataset):
        """
        :param raw_data_dir:
            Computer-specific directory where raw data is stored.
        ...
        self._data_path = os.path.join(
            self.raw_data_dir, self.date, self.name)
        self._out_path = os.path.join(
            self.processed_data_dir, self.date, self.name)
        self._roi_coords_file = os.path.join(
            self._out_path, 'roi_coords.npz')
        self._led_frame_file = os.path.join(
            self._out_path, 'led_frames.npz')
        self._keypoints_file = os.path.join(
            self._out_path)
        self._shifts_file = os.path.join(
            self._out_path, 'shifts.npz')
    ...
    def get_motion(crop):
        ...
        if nframes < 1:
            strr = ('In get_motion(), crop.shape[0]' +
                    ' must be > 0. Shape is currently: {}')
            raise(ValueError, strr.format(crop.shape))
    ...
    def run_cnmfe(self,
        ...
        for idx, inp in enumerate(input_folders):
            ...
            cmd = (matlab_path +
                   ' -softwareopengl -nodesktop -nosplash -r \"' +
                   cnmfe_function +
                   '(\'' + inp + '\', \'' +
                   out_name + '\', ' + str(nthreads) + '); exit;\"')
