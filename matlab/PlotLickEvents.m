function PlotLickEvents(str)
    name = 'scratch';
    load(str)
    mouse=name;

    %% Define constants and parameters
    lag = 4; % if selected trials, only keep CORRECT trials >= lag trials since last change?
    three_spouts = true; % THREE or FOUR spouts
    select_trials = false; % show only good trials
    correct_condition = false; % if good trials, only show correct and >= lag or just >= lag
    ...
    figure('position', [-69, 1312, 550, 404]); clf;
    set(gcf,'Color','w');
    ...
    %% Make polar plot
    figure('position', [497, 1317, 850, 400]);
    set(gcf,'Color','w')
    colors = 'krgy';
