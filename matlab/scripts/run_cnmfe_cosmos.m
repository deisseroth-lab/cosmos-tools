function neuron = run_cnmfe_cosmos(nam, out_name, num_threads)
...
%     disp(['Num threads: ', num2str(num_threads)])
%     maxNumCompThreads(num_threads)
%     maxNumCompThreads(num_threads)
    p = parpool(num_threads);
    p.IdleTimeout = 120;
    disp(p)
    ...
    %% parameters
    % -------------------------    COMPUTATION    -------------------------  %
    pars_envs = struct('memory_size_to_use', 200, ...   % GB, memory space you allow to use in MATLAB %THIS IS APPARENTLY NOT USED!
        'memory_size_per_patch', 10, ...   % GB, space for loading data within one patch
        'patch_dims', [64, 64], ...
        'batch_frames', []);  %GB, patch size [64, 64]
...
%         neuron.update_spatial_parallel(use_parallel, true);
    ...
    if size(neuron.A, 2) == 0
        disp('NO NEURONS FOUND AFTER MERGE WITH CURRENT INITIALIZATION SETTINGS')
        return
    end
...
tic
save([neuron.P.folder_analysis, '/', out_name, '_out.mat'],'-v6','-struct','neuron');
toc

disp(num2str(toc(start_time)))
end
